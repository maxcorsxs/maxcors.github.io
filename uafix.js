 ...
 * 
 * Оновлення:
 * - Додано російську озвучку (RU)
 * - Мови відображаються прапорами: 🇺🇦, 🇬🇧, 🇵🇱, 🇷🇺
 */
(function () {
    'use strict';
    ...
    function setupMarksSettings() {
        ...
        Lampa.SettingsApi.addParam({ component: 'likhtar_marks', param: { name: 'likhtar_badge_ru', type: 'trigger', default: true }, field: { name: 'Русская озвучка (RU)' } }); // новий пункт
    ...
    // =================================================================
    // LIKHTAR QUALITY MARKS (Jacred + UaFix + RuFix)
    // =================================================================
    function initMarksJacRed() {
        ...
        function getBestJacred(card, callback) {
            ...
            fetchWithProxy(apiUrl, function (err, data) {
                ...
                try {
                    ...
                    var best = { resolution: 'SD', ukr: false, eng: false, ru: false, hdr: false }; // додано ru
                    ...
                    results.forEach(function (item) {
                        ...
                        // Російська (нове)
                        if (t.indexOf('rus') >= 0 || t.indexOf('ru') >= 0 || t.indexOf('russian') >= 0 || t.indexOf('рус') >= 0) {
                            best.ru = true;
                        }
        ...
        // Оновлено: тепер виводимо флаги для всіх мов, що є в даних
        function renderInfoRowBadges(container, data) {
            ...
            if (data.pl) {
                var plTag = $('<div class="full-start__pg"></div>');
                plTag.text('🇵🇱');
                container.append(plTag);
            }
            if (data.ru) {
        ...
        function addMarksToContainer(element, movie, viewSelector) {
            ...
            // Якщо є статичні дані (наприклад, від іншого плагіна)
            if (movie.has_ua !== undefined || movie.quality !== undefined) {
                var staticData = {
                    ukr: movie.has_ua === true,
                    pol: movie.has_pl === true,
                    ru: movie.has_ru === true, // додано про всяк випадок
        ...
        function renderBadges(container, data, movie) {
            ...
            // Мовні прапорці (враховуємо налаштування)
            if (data.ukr && Lampa.Storage.get('likhtar_badge_ua', true)) container.append(createBadge('ua', '🇺🇦'));
            if (data.pol && Lampa.Storage.get('likhtar_badge_pl', true)) container.append(createBadge('pl', '🇵🇱'));
            if (data.eng && Lampa.Storage.get('likhtar_badge_en', true)) container.append(createBadge('en', '🇬🇧'));
            if (data.ru && Lampa.Storage.get('likhtar_badge_ru', true)) container.append(createBadge('ru', '🇷🇺'));
        ...
        style.innerHTML = `
            ...
            .card__mark--ru  { background: linear-gradient(135deg, #1e3c72, #d52b1e); color: #fff; border-color: rgba(255,255,255,0.3); } /* новий стиль для RU */
